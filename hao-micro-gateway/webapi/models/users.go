...
func (u *Users) GetRow() bool {
	has, err := config.MySQLEngine.Get(u)
	if err == nil && has {
		return true
	}
	return false
...
func (u *Users) GetAll() ([]Users, error) {
	var users []Users
	err := config.MySQLEngine.Find(&users)
	return users, err
}

func (u *Users) Add(trace *Trace, device *Device) (int64, error) {
	session := config.MySQLEngine.NewSession()
	defer session.Close()
...
func (u *Users) GetRowById() (UserRow, error) {
	var userRow UserRow
	_, err := config.MySQLEngine.Table(usersTable).Where("id=?", u.Id).Get(&userRow)
	return userRow, err
}
