...
func SetDefaultInt(value, def, defValue int) int {
	if value == def {
		return defValue
	}
	return value
...
func InArrayStringHasPrefix(v string, m *[]string) bool {
	for _, value := range *m {
		if value == v {
			return true
		}
		if strings.HasPrefix(strings.ToLower(v), strings.ToLower(value)) {
			fmt.Println("-->>>>>00000>>>>>valuevaluevalue111>>>", value)
